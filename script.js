...
// Sistema de cálculo de frete baseado em bairros (sem APIs externas)
function calculateDeliveryFeeByNeighborhood(neighborhood) {
    // Normaliza o nome do bairro (remove acentos, converte para minúsculo)
    ...
    const neighborhoodFees = {
        ...
        // Zona 3: R$ 8,00
        'barra de jangada': 8.00,
        
        // Zona 4: R$ 10,00
        'estrada de curcurana': 10.00,
        'guararapes': 10.00,
    ...
    // Busca o bairro no mapeamento
    const fee = neighborhoodFees[normalizedNeighborhood];
    
    if (fee !== undefined) {
        return {
            fee: fee,
            neighborhood: neighborhood,
            zone: getZoneByFee(fee)
        };
    }
    
    // Se não encontrar o bairro, retorna taxa padrão
    return {
        fee: 8.00,
        neighborhood: neighborhood,
        zone: 'Bairro não mapeado'
    };
}

// Função auxiliar para determinar a zona baseada na taxa
function getZoneByFee(fee) {
    if (fee === 5.00) return 'Zona 1 - Próximo';
    if (fee === 6.00) return 'Zona 2 - Médio';
    if (fee === 8.00) return 'Zona 3 - Distante';
    if (fee === 10.00) return 'Zona 4 - Longe';
    if (fee === 15.00) return 'Zona 5 - Muito Longe';
    return 'Zona Padrão';
}
...
function extractNeighborhoodFromAddress(address) {
    // Lista de bairros para busca
    const neighborhoods = [
        'loreto', 'jardim piedade', 'dom helder', 'cajueiro seco', 'cadeias', 'prazeres',
        'barra de jangada', 'estrada de curcurana', 'guararapes', 'setubal', 'boa viagem', 
        'imbiribeira', 'ipsep'
    ];
    
    // Normaliza o endereço (remove acentos, converte para minúsculo)
    const normalizedAddress = address
        .toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '');
    
    // Busca cada bairro no endereço
